...
import { EquipmentItem, UserProfile } from '../types';
    ...
    <div className="max-w-6xl mx-auto space-y-6 text-left">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Track tactical radios, body worn cameras, ballistic vests, and security kit issuance
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredEquipment.map((item) => (
          <div
            key={item.id}
            className="bg-white border border-slate-200 hover:border-red-200 rounded-3xl p-5 shadow-sm flex flex-col justify-between transition group"
          >
            <div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
              {item.status === 'available' ? (
                <button
                  ...
                  className="w-full py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-bold transition shadow-sm"
                >
                  Issue to Officer
                </button>
              ) : item.status === 'issued' ? (
                <button
                  ...
                  className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 rounded-xl text-xs font-bold transition"
                >
                  Log Return / Check-In
                </button>
              ) : (
                <div className="text-xs text-amber-700 font-medium py-1.5 text-center w-full">
                  Undergoing Maintenance Check
                </div>
              )}
            </div>
          </div>
